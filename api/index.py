    ...
    try:
        if method == 'GET':
            if path == '/':
                ...
                version_content = app.layout('Outlook Desktop Active', app.top('Outlook Desktop Active', f'You are running Smart Export CRM V7.2 on Vercel.', '<a class="btn" href="/">Dashboard</a>') + '<div class="card good"><h2>Outlook desktop integration is active</h2><p>Email buttons now use the Windows MAILTO protocol to open the installed Outlook app with To, Subject and Message pre-filled. Outlook Web is not used.</p><p class="hint">If another mail app opens, set Microsoft Outlook as the Windows default for MAILTO.</p></div>', '')
                ...
                return html_response(app.error_page('Page not found'), '404 Not Found')
        ...
        return html_response(app.error_page(str(e)), '500 Internal Server Error')
