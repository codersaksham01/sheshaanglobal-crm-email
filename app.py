...
def campaigns_page():
    with conn() as c:
        rows=c.execute("""SELECT ca.id,ca.name,ca.template_id,ca.segment_note,ca.created_at,ca.updated_at,et.name template_name,COUNT(cr.id) recipients,COALESCE(SUM(CASE WHEN cr.sent=1 THEN 1 ELSE 0 END),0) sent_count,COALESCE(SUM(CASE WHEN cr.reply_received=1 THEN 1 ELSE 0 END),0) reply_count FROM campaigns ca LEFT JOIN email_templates et ON et.id=ca.template_id LEFT JOIN campaign_recipients cr ON cr.campaign_id=ca.id GROUP BY ca.id,ca.name,ca.template_id,ca.segment_note,ca.created_at,ca.updated_at,et.name ORDER BY ca.created_at DESC""").fetchall()
